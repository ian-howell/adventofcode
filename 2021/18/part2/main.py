...
def main():
    ...
    for i, xi in enumerate(orig_snail_numbers):
        for j, xj in enumerate(orig_snail_numbers[i+1:], i+1):
            current_magnitude = magnitude(snail_add(deepcopy(xi), deepcopy(xj)))
            best_magnitude = max(best_magnitude, current_magnitude)
            current_magnitude = magnitude(snail_add(deepcopy(xj), deepcopy(xi)))
            best_magnitude = max(best_magnitude, current_magnitude)
